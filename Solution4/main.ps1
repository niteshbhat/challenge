...
try {
    ...
    #New Session has been activitated 
    try {
    ...
    }else { write-host "we unable to login into the session"}
    ...
    write-host "Reason is retrive the password 'Thanks for the challenge' and Username: "$($accountPassword).UserName" & Password: "$($accountPassword).Password""
   ...
   #close PASSession which has been close
    Close-PASSession
}
